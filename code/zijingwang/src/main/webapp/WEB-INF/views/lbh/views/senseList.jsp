...
function de(senseId){
	...
	$.get('/zijingwang/senseDelete2',	{'senseId':senseId}, function(data){
	
		if(data == 1){ 
			alert("delete succeed");
			window.location.href = "/zijingwang/listSense";
		}
					...
					<div class='row-fluid'>
						<div class='span12'>
							<div class='page-header'>
								<h1 class='pull-left'>
									<i class='icon-edit'></i> <span>敏感词管理</span>
								</h1>
							</div>
						</div>
					</div>
							...
							<div class='box-content box-no-padding'>
												...
												<c:choose>
													<c:when test="${page ne null and page.list ne null }">
														<c:forEach items="${page.list }" var="s">
															<tr>
																<td>${s.senseId }</td>
																<td>${s.senseContent }</td>
																<td><button class="btn btn-warning" name="button"
																		style="margin-bottom: 5px" type="submit"
																		onclick="location.href='/zijingwang/senseUpdate?senseId=${s.senseId}'">编辑</button></td>
																<td><button class="btn btn-danger" name="button"
																		style="margin-bottom: 5px" type="submit"
																		onclick="de(${s.senseId})">删除</button></td>
															</tr>
														</c:forEach>
								...
								<div id="p">
											<c:if test="${page.current gt 1 }">
												<a href="/zijingwang/listSense?cur=${page.current-1 }">上一页</a>
											</c:if>
											<c:forEach begin="1" end="${page.total }" var="cur">
												<a href="/zijingwang/listSense?cur=${cur }">${cur }</a>
											</c:forEach>
											<c:if test="${page.current lt page.total }">
												<a href="/zijingwang/listSense?cur=${page.current+1 }">下一页</a>
											</c:if>
										</div>
							</div>
						</div>
