							...
							<div class='box-content box-no-padding'>
														...
														<c:forEach items="${page.list }" var="a">
															<tr>
																...
																<td><button class="btn btn-success" name="button"
																		...
																		onclick="location.href='/zijingwang/articleDetail?articleId=${a.articleId}'">查看</button></td>
																<td><button class="btn btn-danger" name="button"
																		style="margin-bottom: 5px" type="submit"
																		onclick="location.href='/zijingwang/articleDelete?articleId=${a.articleId}'">删除</button></td>
																<td><button class="btn btn-warning" name="button"
																		style="margin-bottom: 5px" type="submit"
																		onclick="location.href='/zijingwang/articleUpdate?articleId=${a.articleId}'">编辑</button></td>
															</tr>
														</c:forEach>
													</c:when>
													<c:otherwise>
								...
								<div id="p">
											<c:if test="${page.current gt 1 }">
												<a href="/zijingwang/listArticle?cur=${page.current-1 }">上一页</a>
											</c:if>
											<c:forEach begin="1" end="${page.total }" var="cur">
												<a href="/zijingwang/listArticle?cur=${cur }">${cur }</a>
											</c:forEach>
											<c:if test="${page.current lt page.total }">
												<a href="/zijingwang/listArticle?cur=${page.current+1 }">下一页</a>
											</c:if>
										</div>
							</div>
						</div>
